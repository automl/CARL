...
def setup_agent(config, outdir, parser, args):
    ...
    contexts = sample_contexts(
        env, context_args, num_contexts, default_sample_std_percentage=0.1
    )
...
parser.add_argument("--policy_path", help="Path to PBT policy")
...
model, timesteps, context_args, hide_context = setup_agent(
    config, args.outdir, parser, args
)
change_at, next_config = next(hp_schedule, None)
for i in range(250):
    config["policy_kwargs"] = {"net_arch": [args.net_size, args.net_size]}
    reward, model, timesteps = step(
        model, timesteps, args.env, context_args, hide_context
    )
