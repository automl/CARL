...
iteration_list = [
    ...
    {"name": "default_sample_std_percentage", "id": "std", "values": [0.1, 0.25, 0.5]},
    {"name": "hide_context", "id": "hid", "values": [True, False]},
    {
        "name": "context_feature_args",
        "id": "cfargs",
        "values": ["None"]
        + list(env_defaults.keys()),  # None trains only with the default context
    },
]
#########################################################################
if follow_evaluation_protocol:
...
if use_cpu:
    ...
    gres = "gpu:1"
mail_user = (
    "benjamin@tnt.uni-hannover.de" if not on_luis else "benjamins@tnt.uni-hannover.de"
)
output_filename = "slurmout/slurm-%j.out"
mem_per_cpu = "2000M" if "Racing" not in env else "8000M"
basecommand = (
    f"{xvfb_str}python {runfile} --num_contexts 100 --steps {n_timesteps} "
...
if on_luis:
    ...
    conda_env_name = "carl"
    project_name = "CARL"
    slurmout_dir = os.path.join(
        "$BIGWORK", project_name, "slurmout"
    )  # need to define it manually so far :( # TODO define once per sh, maybe in runcommands.sh
    slurmout_dir = os.path.join(
        "/bigwork/", luis_user_name, project_name, "slurmout"
    )  # cannot use environment variables in #SBATCH definitions :(
    output_filename = os.path.join(slurmout_dir, "slurm-%j.out")
    with open(luis_template_fn, "r") as file:
        content = file.read()
    pre_command = content.format(
...
basecommand += (
    f" --outdir {outdir}  --num_workers {cpus_per_task} --build_outdir_from_args"
)
basecommand += (
    f" --eval_freq {eval_freq} --seed $SLURM_ARRAY_TASK_ID "
...
is_hide_context_in_iter = np.any(
    [el["name"] == "hide_context" for el in iteration_list]
)
