...
def create_ep_contexts_LUT(
    ...
    seeds: List[int],
    modes: List[str],
    contexts_LUT_fn: Optional[Union[str, Path]] = None,
) -> pd.DataFrame:
    index_col = ["mode", "seed", "context_distribution_type", "instance_id"]
    contexts_LUT = []
    for mode in modes:
        for seed in seeds:
            context_dict = get_ep_contexts(
                env_name=env_name, n_contexts=n_contexts, seed=seed, mode=mode
            )
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        for mode in modes:
            contexts_train = get_train_contexts(
                env_name=env_name, seed=seed, mode=mode, n_contexts=n_contexts
            )
