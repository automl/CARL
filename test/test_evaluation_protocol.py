...
from experiments.evaluation_protocol.plot_evaluate_on_protocol import (
...
class TestEvaluationProtocol(unittest.TestCase):
    def test_context_creation(self):
        cf0 = ContextFeature("g", 9.0, 9.5, 10.0, 11.0)
        ...
        for mode in modes:
            ep = EvaluationProtocol(
                context_features=context_features, mode=mode, seed=seed
            )
            ...
            for c_id, C in contexts_dict.items():
                if len(C) != 0:
                    self.assertTrue(
                        len(C) == n_contexts,
                        msg=f"Number of contexts {len(C)} not equal to desired number {n_contexts} for {c_id}.",
                    )
