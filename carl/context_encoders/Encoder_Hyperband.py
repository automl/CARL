...
def main (cfg: DictConfig) -> None:
    ...
    out_dir = cfg.encoder.outdir

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    dataset = np.load(cfg.encoder.context_db)
    ...
    def train_model(model, **kwargs):
        ...
        loader = th.utils.data.DataLoader(dataset = train_set,
        ...
        ## Validation 
        loader = th.utils.data.DataLoader(dataset = val_set,
                                        batch_size = kwargs['batch'],
                                        shuffle = False)
    ...
    strategy = HyperbandSearch(
                ...
                integer={"batch": {"begin": 30,
                                    "end": 128,
                                    "prior": "log-uniform"}
                        },
                search_config={"max_resource": 14,
                                "eta": 3},
                seed_id=42,
            )
