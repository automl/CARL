...
class TukTuk(Car):
    ...
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            angle=init_angle,
            fixtures=[
                fixtureDef(shape=polygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in BHULL_POLY2]), density=1),
                fixtureDef(shape=polygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in BHULL_POLY3]), density=1)
                ]
            )
        self.hull.color = (0.2, 0.5, 1.)
        self.wheels = []
        self.fuel_spent = 0.0
        WHEEL_POLY = [
...
class TukTukSmallTrailer(TukTuk):
    ...
    def _init_extra_params(self):
        ...
        self.trailer_type = 1  # Determines which trailer to attach 0 -> none, 1 -> small
