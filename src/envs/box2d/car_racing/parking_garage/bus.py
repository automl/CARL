...
FRICTION_LIMIT = 1_250_000*SIZE*SIZE     # friction ~= mass ~= size^2 (calculated implicitly using density)
...
class Bus(Car):
    ...
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            angle=init_angle,
            fixtures=[
                fixtureDef(shape=polygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY1]), density=.66),
                fixtureDef(shape=polygonShape(vertices=[(x*SIZE, y*SIZE) for x, y in HULL_POLY2]), density=.2)
                ]
            )
        self.hull.color = (1., 0.85, 0.)
        self.wheels = []
        self.fuel_spent = 0.0
        WHEEL_POLY = [
